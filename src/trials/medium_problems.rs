/*
    ...
    - Using a fixed-size array instead of HashMap (memory: 5.5MB -> 4.12MB, runtime: 0.3s -> 0.04s)
        - this specifically causes the main thread to go out of stack on local machine for k = 1000
        - setting sizes to 100 so that main thread doesn't go out of stack
*/
